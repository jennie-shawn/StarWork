...
普通 stable / latest 用户不从这里安装；稳定版本使用 `product/skills/`。
...
| 层级 | 位置 | 是否全局安装 | 作用 |
|---|---|---:|---|
| L0 主入口 | `product/skills-next/starwork/` | 是 | StarWork 产品解释、安装引导、模糊意图路由 |
| L1 系统专家 | `product/skills-next/starworkInit/` 等 | 是 | 初始化、诊断、知识库、多 Agent 与 workflow next 等明确能力 |
...
- Stable 系统级 Agent 工作流 skill 放在 `product/skills/`。
- Workflow next 内测系统级 skill 放在 `product/skills-next/`。
...
- `starworkMultiagent/`：帮助 Agent 把“常用智能体 / 当前会话职责 / 多 Agent 分工 / 共享输出 / 跨会话指令 / lane session 编排 / workflow Builder / Runner”翻译成安全的 StarWork 协作流程；Codex App 正常路径继续直接使用标准线程工具。详细 SPEC 见 `starworkMultiagent-spec.md`、`product/planning/features/multiagent/specs/v0.10-upgrade-migration-compatibility.md` 和 `product/planning/features/multiagent/specs/v0.11-workflow-builder-runner-mvp.md`。
