...
下一步重点不是继续扩张 Core 或宿主自动化，而是进入 M2.12 发布与 A 测稳定化：同步 `alpha.20` 发布口径、补 release checklist、收窄 A 测主流程，并打磨 `init`、`doctor`、`upgrade`、`knowledge`、项目中心管理、MultiAgent 和 Host Adapter 的真实使用链路；入口稳定后，再补 Golden Demo Workspaces，最后进入内容创作者 Pack 的场景验证。
