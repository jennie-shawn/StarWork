...
当用户说“初始化多 Agent 协作层”“创建 Agent 团队”“帮我创建多个智能体”时，先讲清楚这一步是什么，不要直接创建 lane：

```text
多 Agent 分工是把一个项目里的不同 AI 会话按职责分开协作。

它能帮你：
- 让产品规划、开发、验收等职责边界更清楚；
- 让每个 Agent 只主动修改自己被授权的文件范围；
- 通过共享输出和跨会话消息交接上下文。

接下来我会先设计职责，再创建或绑定会话，最后记录到 StarWork 协作状态里。

整个过程中，我会先预览职责和写入范围；只有确认能创建并绑定可工作的独立会话时，才说 Agent 团队创建完成。
```
