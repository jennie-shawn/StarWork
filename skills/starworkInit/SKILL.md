...
Step 5 询问是否需要项目知识库（默认不需要）
Step 6 采访正式成果放哪里
Step 7 采访日常工作在哪里发生
Step 8 采访额外固定区域和 Agent 规则
```
...
只有 Step 1 是 Project 时才问，并且放在工作台类型、语言、路径之后。

推荐问法：

```text
这个项目需要让 AI 长期整理“项目知识库”吗？

知识库不是放原始资料的文件夹，而是让 AI 把长期有用的主题、判断和方法沉淀下来。
如果你现在还不确定，先不开启也没关系，之后可以再运行 starwork knowledge init。
```

默认选“不需要”。如果用户选择需要，初始化命令增加 `--knowledge`。项目中心不问这个问题；项目中心共享知识库后续单独设计。
