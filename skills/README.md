...
这里存放 stable / latest 用户通过下面命令安装到 Agent 全局环境的 StarWork 系统级 skill：

```bash
npx skills add https://github.com/jennie-shawn/StarWork/tree/main/skills --full-depth -g -a codex -y
```

必须保留 `--full-depth`，确保 `starworkMultiagent/references/` 等配套文件随 `SKILL.md` 一起安装。只安装主文件会导致高风险 MultiAgent 动作停止并提示 Skill 安装不完整。

Workflow next 内测不从这里安装；内测用户使用 `skills-next/`。
...
| 层级 | 位置 | 是否全局安装 | 作用 |
|---|---|---:|---|
| L0 主入口 | `skills/starwork/` | 是 | StarWork 产品解释、安装引导、模糊意图路由 |
| L1 系统专家 | `skills/starworkInit/` 等 | 是 | 初始化、诊断、知识库、多 Agent 等明确能力 |
| L2 Kit 自带 | `kit-skills/` | 否 | 项目中心或项目工作台形态自带能力 |
...
- 系统级 Agent 工作流 skill 放在 `skills/`。
- Kit 自带 skill 放在 `kit-skills/`。
...
- `starworkMultiagent/`：帮助 Agent 把“常用智能体 / 当前会话职责 / 多 Agent 分工 / 共享输出 / 跨会话指令 / lane session 编排”翻译成安全的 StarWork 协作流程；主入口较短，长流程在 `starworkMultiagent/references/` 中按场景加载。详细 SPEC 见 `starworkMultiagent-spec.md`。
