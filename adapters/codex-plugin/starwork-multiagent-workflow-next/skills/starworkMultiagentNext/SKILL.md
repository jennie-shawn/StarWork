---
name: starworkMultiagentNext
description: 'Experimental plugin-bundled entry for StarWork MultiAgent Workflow Next. Use explicitly as $starworkMultiagentNext to design or run next workflow in a StarWork workspace.'
starwork_channel: next
starwork_multiagent: v0.15-codex-plugin-adapter
starwork_plugin_adapter: codex
---

# starworkMultiagentNext

这是 Codex plugin bundled 的实验性入口。MVP 阶段请显式使用 `$starworkMultiagentNext`，避免触发用户级或仓库级旧 `starworkMultiagent`。

使用这个 Skill，把用户关于“设计 workflow”“启动 workflow”“当前会话职责”“多 Agent 分工”“跨 Agent 输出共享”“跨会话指令”“查看其他 lane 进度”的自然语言请求，转成安全的 StarWork workflow next 协作流程。

`starworkMultiagentNext` 不是 `starwork multiagent` 命令本身。Skill 负责判断意图、读取必要 reference、直接调用宿主标准会话工具，并把真实结果记录回 StarWork 项目事实源。**CLI 只做 StarWork 项目事实源**，不是宿主会话动作执行者。

这个 Codex plugin adapter 只提供 Codex 侧安装、发现、显式触发和 bundled Skill / references packaging；不替代 StarWork CLI、Core、`.starwork/`、`_系统/协作/` 或 workflow run state，也不自带 `create_thread`、`send_message_to_thread`、`read_thread` 等 Codex 标准线程工具。

如果用户只是询问 StarWork 是什么、怎么开始、安装入口或该用哪个能力，回到 `starwork` 主入口。用户明确说多 Agent、Agent Lanes、lane、跨会话消息、开发 Agent、产品 Agent、验收 Agent、Codex 标准会话工具、Codex 会话控制或 workflow 时，继续使用本 Skill。
...
- CLI 必须来自 `@jennie-shawn/starwork@next`。
- 当前 plugin bundled Skill 必须包含 workflow references；如果缺失，说明 plugin 安装不完整，停止 workflow 操作，提示用户重新安装 plugin。
...
如果 reference 文件不存在或无法读取，不得继续执行对应高风险动作；先说明 Skill 安装不完整，并要求用户用完整目录重新安装 StarWork next Skills。
...
任何会话控制、workflow Runner 或跨会话操作前，必须确认**当前会话 ID**。
